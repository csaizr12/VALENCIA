import pandas as pd
import matplotlib
matplotlib.use('Agg') # Necesario para pipelines/servidores
...
def generate_quality_panel(gff_path, output_png):
    print(f"📊 Generando Panel de Calidad (Alineación Perfecta + Magma) desde: {gff_path}")
    data = []
    try:
        ...
        # UBICACIÓN DESPLAZADA HACIA ABAJO (Fila 4) de la colorbar
        ...
        # --- 2. PLOT B y C (Se mantienen igual) ---
        ...
        plt.close()
        print(f"✅ Reporte de Calidad de Alto Contraste generado: {output_png}")
