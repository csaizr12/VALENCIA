...
def generate_quality_panel(gff_path, output_png):
    ...
    try:
        ...
        with open(gff_path, 'r', encoding='latin-1') as f:
          for line in f:
            ...
            if fields[2].lower() in ['mrna', 'transcript']:
             attr = fields[8]
             # Usamos el nuevo patrón para capturar notación científica correctamente
             ...
             if tx and pr and cds:
                # Ahora float() interpretará correctamente el string con la "e"
                t_v = float(tx.group(1))
                p_v = float(pr.group(1))
                c_v = float(cds.group(1))
        ...
        inner_gs = gs[0].subgridspec(7, 4,height_ratios=[1.2, 1, 1, 1, 1, 0.6, 0.2],hspace=0.05, wspace=0.05)
        ...
        leg_elements = [Patch(facecolor='#45a049', label='Lev_edit_distance transcripts'),Patch(facecolor='#e91e63', label='Lev_edit_distance proteins')]
        ax_hist_y.legend(handles=leg_elements, loc='upper left',bbox_to_anchor=(1.05, 1.15), frameon=True, fontsize=11)
