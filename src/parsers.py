    ...
    with open(refmap_path, "r") as f:
        for line in f:
            if line.startswith("ref_gene"):
                continue
            fields = line.strip().split('\t')
            # get the class code from the 3rd column and translate 
            # it to a more descriptive term using the CLASS_CODE_TRANSLATION dictionary
            ...
            for gene in fields[3].split(","):
                ...
                # if the gene is not found in the gene_isoform_dict, we skip it;
                #  otherwise, we look for the isoform and update its evidence information
                if target_gene is None: 
                    continue
                if target_gene:
                    if iso_id in target_gene:
                        target_gene[iso_id].update({evidence_type: {"class_code":class_code, 
                                                                    "match_sequence": fields[1]}})
